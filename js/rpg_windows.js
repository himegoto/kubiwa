...
if(actor!=$gameParty.members()[1]){
width = width || 186;
    var color1 = this.hpGaugeColor1();
    var color2 = this.hpGaugeColor2();
	
	var yy=y;
	if(actor==$gameParty.members()[3]){yy-=40;}
    this.drawGauge(x, yy, width, actor.hpRate(), color1, color2);
    this.changeTextColor(this.systemColor());
	
	var txt=TextManager.hpA;yy=y;
	if(actor==$gameParty.members()[2]){txt="尿意";}else if(actor==$gameParty.members()[3]){txt="空腹度";yy-=40;}
    this.drawText(txt, x, yy, 44);////////////////////////////////////////////////////
    this.drawCurrentAndMax(actor.hp, actor.mhp, x, yy, width,
...
if(actor!=$gameParty.members()[1]){	
	...
	var yy=y;
	if(actor==$gameParty.members()[3]){yy-=40;}
    this.drawGauge(x, yy, width, actor.mpRate(), color1, color2);
    this.changeTextColor(this.systemColor());
	
	var txt=TextManager.mpA;yy=y;
	if(actor==$gameParty.members()[2]){txt="周期";}else if(actor==$gameParty.members()[3]){txt="水分";yy-=40;}
    this.drawText(txt, x, yy, 44);//////////////////////////////////////////////////
    this.drawCurrentAndMax(actor.mp, actor.mmp, x, yy, width,
...
Window_MenuStatus.prototype.initialize = function(x, y) {
    var width = this.windowWidth();
    var height = this.windowHeight()+75;
    Window_Selectable.prototype.initialize.call(this, x, y, width, height);
